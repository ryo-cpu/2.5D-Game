...
const float WorldSprite::GetRag() const
{
    float R = VSize(VSub(Apex[0], Apex[2]));
    VECTOR Y=  VSub(Apex[1], Apex[0]);
    VECTOR X = VSub(Apex[2], Apex[0]);

    float Ysize=VSize (VSub(Apex[1], Apex[0]));
    float Sin = Ysize / R;
    float Rag;
    if (Y.y < 0)
    {
